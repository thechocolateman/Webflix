...
import { activeMovie } from "../../state/movies";
...
export default function Card(props) {
  ...
  function getVideo(action, id) {
    switch (action) {
      case "add":
        let url = "";
        fetch(
          `https://api.themoviedb.org/3/movie/${id}/videos?language=en-US&api_key=55c56b9f930280a2563491ffe49d383f`
        )
          .then((res) => res.json())
          .then((json) =>
            setUrl(`https://www.youtube.com/watch?v=${json.results[0].key}`)
          )
  ...
  return (
    <motion.div
      key={props.itemKey}
      id={`movie-${props.itemKey}`}
      className={`carousel__img__container`}
      whileHover={{
        scale: 1.5,
        transition: { duration: 0.4 },
      }}
      onHoverStart={(e) => {
        e.target.className === "carousel__img__container"
          ? (e.target.querySelector(".modal__container").style.display =
              "block")
          : console.log("erfde");
        // console.log(e.target)}
        try {
          document.getElementsByClassName("next-navigation")[0].style.display =
            "block";
      ...
      onHoverEnd={(e) => {
        e.target.className === "carousel__img__container"
          ? (e.target.querySelector(".modal__container").style.display = "none")
          ...
          <div className="down-icon">
            <Icon
              ...
              onClick={(e) => {
                ...
                e.currentTarget.parentElement.closest(
                  ".modal__container"
                ).style.display = "block";
              }}
            />
