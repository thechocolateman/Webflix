...
import { saveMovie } from "../state/movies";
import { saveCategory } from "../state/categories";
      ...
      case "tv":
        for (const element in genreState) {
          genreState[element]
            ...
            .map((id) => {
              fetch(
                `https://api.themoviedb.org/3/discover/tv?include_adult=false&include_null_first_air_dates=false&with_original_language=en&watch_region=us&language=en-US&page=1&with_genres=${id}&sort_by=popularity.desc&api_key=${API_KEY}`
              )
                .then((data) => data.json())
                .then((json) =>
                  dispatch(saveTvSeries({ id: id, results: json.results }))
                );
            });
        }
      case "movies":
        for (const element in genreState) {
          ...
          genreState[element]
            ...
            .map((id) => {
              fetch(
                `https://api.themoviedb.org/3/discover/movie?include_adult=false&include_null_first_air_dates=false&with_original_language=en&watch_region=us&language=en-US&without_genres=99&page=1&with_genres=${id}&sort_by=popularity.desc&api_key=${API_KEY}`
              )
                .then((data) => data.json())
                .then((json) => {
                  // console.log("SAVED")
